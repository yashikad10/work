...
export async function POST(req: NextRequest) {
  try {
    ...
    const processInscriptionsData = await processInscriptions(order_id, imageInfo, 262, "testnet");
    const dataToSave = {
    ...
    await dbConnect();
   const datadb= await FileData.create(dataToSave)
    console.log(datadb);
...
//     await dbConnect();
//     // Create the document
//     const newDocument = await FileData.create(dataToSave);
//     await dbConnect();
