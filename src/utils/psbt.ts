...
import { CreateInscription} from "@/types";
...
const changeValue =
totalValue -
(inscriptions[0].inscription_fee+ finalFees) -  //(order.inscription_fee -finalfees)
...
inscriptions.map((i: CreateInscription) => {
psbt.addOutput({
...
export async function generateUnsignedPsbtForInscription(
    ...
    try {
      paymentUtxos = await selectPaymentUTXOs(
        ...
        order.inscription_fee  + 10000,  //inscription_fee instead of 10000 
        Math.floor(Math.random() * 3) + 3, // number between 3-5
        inscriptions.length + 1, //change + inscription
        fee_rate,
